...
void SyntacticAnalyzer::analyze() {
	ofstream out;
	out.open("./gen_data/AnalysisProcess.csv", ios::out);
	if (out.fail()) {
	...
	if (la.openFile(codeFilename, resultFilename))
	{
		...
		while (true) {
			...
			while (true) {
				...
				if (LR1AnalysisTable.find({ currState,word.wordStr }) == LR1AnalysisTable.end()) {
					cerr << "we meet an error in the process of Syntactic Analysis!" << endl;
					cerr << "There is no match for current state " << currState << " with grammar symbol " << word.wordStr << endl;
					return;
				}
				if (Action::SHIFT == LR1AnalysisTable[{currState, word.wordStr}].action) {//移进操作
					//将此处的分析表中对应的下一个状态压进状态状态序列栈
					stateSequenceStack.push_back(LR1AnalysisTable[{currState, word.wordStr}].nextState);
					//将当前状态面临的符号也压进文法符号栈
					grammarSymbolStack.push_back(word.wordStr);
				...
				else if (Action::REDUCE == LR1AnalysisTable[{currState, word.wordStr}].action) {//规约操作
					//得到产生式序号
					int prodPos = LR1AnalysisTable[{currState, word.wordStr}].nextState;
					//产生式右部长度
					int rightLen = 0;
				...
				else if (Action::ACCEPT == LR1AnalysisTable[{currState, word.wordStr}].action) {//接受状态
					//空产生式
					Production prod;
					vector<string>right;
					string left = "ACCEPT";
					...
					printAnalysisProcess(step, grammarSymbolStack, stateSequenceStack, prod);
					out.close();
					cerr << "Congratulations! Successfully completing analysis!" << endl;
					break;
				}
				else {
