	...
	while (getline(cin, grammarStr)) {
		...
		if (grammarStr != "") {
			...
			while (rightPos != string::npos) {//找到
				...
				//添加终结符的文法符号集合到 grammar_symbols中
...
void SyntacticAnalyzer::getLR1ItemClosure0() {
	...
	set<LR_1_Item>lr1ItemClosure0_;//临时项目集闭包0
	...
	while (!lr1ItemStack.empty()) {
		...
		lr1ItemStack.pop();//出栈
		lr1ItemClosure0_.insert(temp);//存进项目集闭包
		...
		for (auto iter = temp.lookaheadStr.begin(); iter != temp.lookaheadStr.end(); iter++) {
			...
			first.push_back(*iter);//将v插入first之后
		...
		for (auto iter = lr0Items.begin(); iter != lr0Items.end(); iter++) {
			if (iter->pointPos == 0 && prods[iter->prodPos].left == right[pointPos]) {
				if (lr1ItemClosure0_.find({ iter->prodPos,iter->pointPos,lookaheadStr }) == lr1ItemClosure0_.end())
	...
	//此处需要合并相同LR(0)项目 不同展望串的项目 将展望串合并为一个集合
	for (auto iter = lr1ItemClosure0_.begin(); iter != lr1ItemClosure0_.end(); iter++) {
		set<string> lookaheadStr_;
		lookaheadStr_.insert(iter->lookaheadStr.begin(), iter->lookaheadStr.end());
		for (auto iter2 = lr1ItemClosure0_.begin(); iter2 != lr1ItemClosure0_.end(); iter2++) {
			if (iter->prodPos == iter2->prodPos && iter->pointPos == iter2->pointPos) {
				lookaheadStr_.insert(iter2->lookaheadStr.begin(), iter2->lookaheadStr.end());
			}
		}
		lr1ItemClosure0.insert(LR_1_Item{ iter->prodPos,iter->pointPos,lookaheadStr_ });
	}
}
/*
...
set<LR_1_Item> SyntacticAnalyzer::go(const set<LR_1_Item>& lr1Items)
{
	...
	set<LR_1_Item> lr1ItemsClosure_;
	//此处需要合并相同产生式 不同展望串的项目 将展望串合并为一个集合
	for (auto iter = lr1ItemsClosure.begin(); iter != lr1ItemsClosure.end(); iter++) {
		set<string> lookaheadStr_;
		lookaheadStr_.insert(iter->lookaheadStr.begin(), iter->lookaheadStr.end());
		for (auto iter2 = lr1ItemsClosure.begin(); iter2 != lr1ItemsClosure.end(); iter2++) {
			if (iter->prodPos == iter2->prodPos && iter->pointPos == iter2->pointPos) {
				lookaheadStr_.insert(iter2->lookaheadStr.begin(), iter2->lookaheadStr.end());
			}
		}
		lr1ItemsClosure_.insert(LR_1_Item{ iter->prodPos,iter->pointPos,lookaheadStr_ });
	}
	return lr1ItemsClosure_;
}
/*
...
void SyntacticAnalyzer::printLR0Item(const LR_0_Item& lr0Item) {
	printer << prods[lr0Item.prodPos].left << "->";//输出左部
			...
			if (LR1AnalysisTable.find({ i,symbols[j] }) != LR1AnalysisTable.end()) {//LR1分析表中存在该动作和状态的映射
				if (j < terminalSymbols.size()) {//终结符 ACTION
					...
					if (LR1AnalysisTable[{i, symbols[j]}].nextState != -1)
						printer << LR1AnalysisTable[{i, symbols[j]}].nextState << ",";
				}
				else {//非终结符 GOTO
					printer << LR1AnalysisTable[{i, symbols[j]}].nextState << ",";
				}
...
/*
	打印语法分析过程
*/
void SyntacticAnalyzer::printAnalysisProcess(int step, const vector<string>& symbol, const vector<int>& state, const Production& prod) {
	printer << step << ",";
...
void SyntacticAnalyzer::funcPrint() {
	printer.open("./GeneratedData/FirstSet.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/FollowSet.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/LR0Items.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/LR1ItemsNormalFamily.txt");
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/GrammarSymbols.txt");
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/Productions.txt");
	if (printer.fail()) {
	...
	printer.open("./GeneratedData/LR1AnaLysisTable.csv", ios::out | ios::trunc);//清空已有的文件
	if (printer.fail()) {
		cerr << "fail to open file LR1AnaLysisTable.csv!";
	}
	printLR1AnalysisTabel();//打印LR1分析表
	printer.close();
}
...
void SyntacticAnalyzer::build()
{
	getProd("./SourceData/grammar_.txt");//得到单个产生式集合
...
/*
	词法分析一个单词  之后进行语法分析
*/
void SyntacticAnalyzer::analyze(const string& codePath) {
	printer.open("./GeneratedData/SyntacticAnalysisProcess.csv", ios::out);
	if (printer.fail()) {
		cerr << "fail to open file AnalysisProcess.csv!";
	}
	//初始化栈
	stateSequenceStack.push_back(0);
	grammarSymbolStack.push_back("#");
	int step = 0;

	//词法分析
	LexicalAnalyzer la;
	string codeFilename = "SourceData/" + codePath;
	//string codeFilename =codePath;
	string resultFilename = "GeneratedData/LexicalAnalysisResult.csv";
		...
		if (la.packBuffer()) {
			while (true) {
				...
				if (word.type == LEXICAL_TYPE::UNKNOWN) {
					cerr << "Lexical Analysis ERROR!" << endl;
					cerr << "UNKNOWN word " << word.value <<"! Please check it!"<< endl;
					break;
				}
				while (true) {
					//栈顶的状态编号
					int currState = stateSequenceStack.back();//最后一个元素

					if (LR1AnalysisTable.find({ currState,word.name }) == LR1AnalysisTable.end()) {
						cerr << "Syntactic Analysis ERROR!" << endl;
						cerr << "There is no match for current state " << currState << " with grammar symbol " << word.name <<"! Please check it!" <<endl;
						return;
					}
					if (Action::SHIFT == LR1AnalysisTable[{currState, word.name}].action) {//移进操作
						//将此处的分析表中对应的下一个状态压进状态状态序列栈
						stateSequenceStack.push_back(LR1AnalysisTable[{currState, word.name}].nextState);
						//将当前状态面临的符号也压进文法符号栈
						grammarSymbolStack.push_back(word.name);
						...
						prod = { left,right };
						//打印分析过程
						printAnalysisProcess(step, grammarSymbolStack, stateSequenceStack, prod);
						step++;
						GrammarSymbol symbol;
						symbol.name = word.name;
						symbol.value = word.value;
						grammarSymbolAttrStack.push_back(symbol);
						break;//退出while循环 再取一个单词
					}
					else if (Action::REDUCE == LR1AnalysisTable[{currState, word.name}].action) {//规约操作
						//得到产生式序号
						int prodPos = LR1AnalysisTable[{currState, word.name}].nextState;
						//产生式右部长度
						int rightLen = 0;
						if (prods[prodPos].right[0] != "$") {
							rightLen = prods[prodPos].right.size();
						}

						//文法符号栈弹出 要规约产生式右部的所有符号 
						//状态序列栈弹出相应长度个状态
						int i = 0;
						while (i < rightLen) {
							grammarSymbolStack.pop_back();
							stateSequenceStack.pop_back();
							i++;
						}
						//把规约之后的产生式左部压进文法符号栈
						grammarSymbolStack.push_back(prods[prodPos].left);
						//此时的状态对应的符号栈顶的文法符号 再去查分析表
						currState = stateSequenceStack.back();
						string currSymbol = grammarSymbolStack.back();
						//查分析表 找到下一个状态之后压状态栈
						stateSequenceStack.push_back(LR1AnalysisTable[{currState, currSymbol}].nextState);

						if (!semanticAnalyzer.semanticCheck(grammarSymbolAttrStack, prods[prodPos])) {
							return;
						}
						//打印分析过程
						printAnalysisProcess(step, grammarSymbolStack, stateSequenceStack, prods[prodPos]);
						step++;
						//此处不能退出本循环 ！！！！因为还有可能进行规约
					}
					else if (Action::ACCEPT == LR1AnalysisTable[{currState, word.name}].action) {//接受状态
						//空产生式
						Production prod;
						vector<string>right;
						string left = "ACCEPT";
						...
						printAnalysisProcess(step, grammarSymbolStack, stateSequenceStack, prod);
						printer.close();
						cerr << "Congratulations! Successfully complete lexical analysis!" << endl;
						cerr << "Congratulations! Successfully complete syntactic analysis!" << endl;
						cerr << "Congratulations! Successfully complete semantic analysis!" << endl;
						cerr << "Congratulations! Successfully complete target code generation!" << endl;
						break;
					}
					else {
						cerr << "Sorry, Internal error!" << endl;
						return;
					}
				}
				if (word.type == LEXICAL_TYPE::_EOF)//代码文件结尾
					break;
			}
		}
	}
