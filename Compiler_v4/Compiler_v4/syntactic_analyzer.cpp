...
void SyntacticAnalyzer::funcPrint() {
	printer.open("./gen_data/FirstSet.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./gen_data/FollowSet.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./gen_data/LR0Items.txt", ios::out);
	if (printer.fail()) {
	...
	printer.open("./gen_data/LR1ItemsNormalFamily.txt");
	if (printer.fail()) {
	...
	printer.open("./gen_data/GrammarSymbols.txt");
	if (printer.fail()) {
	...
	printer.open("./gen_data/Productions.txt");
	if (printer.fail()) {
	...
	printer.open("./gen_data/LR1AnaLysisTable.csv", ios::out | ios::trunc);//清空已有的文件
	if (printer.fail()) {
...
void SyntacticAnalyzer::analyze(const string& codePath) {
	printer.open("./gen_data/AnalysisProcess.csv", ios::out);
	if (printer.fail()) {
	...
	//词法分析
	LexicalAnalyzer la;
	string codeFilename = "raw_data/" + codePath;
	...
	if (la.openFile(codeFilename, resultFilename))
	{
		...
		while (true) {
			...
			if (word.type == LEXICAL_TYPE::UNKNOWN) {
				cerr << "we meet an UNKNOWN error in the process of Lexical Analysis!" << endl;
				cerr << "The UNKNOWN word is " << word.value << endl;
				break;
			}
			while (true) {
				//栈顶的状态编号
				...
				if (LR1AnalysisTable.find({ currState,word.name }) == LR1AnalysisTable.end()) {
					cerr << "we meet an error in the process of Syntactic Analysis!" << endl;
					cerr << "There is no match for current state " << currState << " with grammar symbol " << word.name << endl;
					return;
				}
				if (Action::SHIFT == LR1AnalysisTable[{currState, word.name}].action) {//移进操作
					...
					printAnalysisProcess(step, grammarSymbolStack, stateSequenceStack, prod);
					printer.close();
					cerr << "Congratulations! Successfully completing lexical analysis!" << endl;
					cerr << "Congratulations! Successfully completing symtactic analysis!" << endl;
					cerr << "Congratulations! Successfully conpleting semantic analysis!" << endl;
					break;
				}
				else {
